...
<article id="post-<?php the_ID(); ?>" <?php post_class( 'card mt-3r' ); ?>>
	<div class="card-body">
		...
		<?php if ( is_sticky() ) : ?>
			<span class="oi oi-bookmark wp-bp-sticky text-muted" title="<?php echo esc_attr__( 'Sticky Post', 'teezr' ); ?>"></span>
		<?php endif; ?>
				...
				<?php
					the_content( sprintf(
						wp_kses(
							...
							__( 'Continue reading<span class="screen-reader-text"> "%s"</span>', 'teezr' ),
							array(
								'span' => array(
									'class' => array(),
								),
					...
					wp_link_pages( array(
						'before' => '<div class="page-links">' . esc_html__( 'Pages:', 'teezr' ),
						'after'  => '</div>',
					) );
				?>
			</div><!-- .entry-content -->
		<?php else : ?>
			<div class="entry-summary">
				<?php the_excerpt(); ?>
				<div class="">
					<a href="<?php echo esc_url( get_permalink() ); ?>" class="btn btn-primary btn-sm"><?php esc_html_e( 'Continue Reading', 'teezr' ); ?> <small class="oi oi-chevron-right ml-1"></small></a>
				</div>
			</div><!-- .entry-summary -->
		<?php endif; ?>
