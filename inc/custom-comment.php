...
function wp_bootstrap_4_comment( $comment, $args, $depth ) {
    ...
    <li <?php comment_class(); ?> id="comment-<?php comment_ID() ?>">
        ...
        ?>
        <article class="media teezr-comment">
            ...
            <div class="media-body">
                <h6 class="mt-0 mb-0 comment-author">
                    ...
                    <?php if ( $comment->comment_author_email == get_the_author_meta( 'email' ) ) : ?>
                        <small class="wb-comment-by-author ml-2 text-muted"><?php echo esc_html__( '&#8226; Post Author &#8226;', 'teezr' ) ?></small>
                    <?php endif; ?>
                </h6>
                <small class="date text-muted"><?php printf( // WPCS: XSS OK.
                                                        /* translators: %1 %2: date and time. */
                                                        esc_html__('%1$s at %2$s', 'teezr'),
                                                        get_comment_date(),
                                                        get_comment_time()
                                                    ); ?></small>
                <?php if ($comment->comment_approved == '0') : ?>
					<small><em class="comment-awaiting text-muted"><?php esc_html_e('Comment is awaiting approval', 'teezr'); ?></em></small>
					<br />
				<?php endif; ?>
                ...
                <small class="reply">
					<?php comment_reply_link( array_merge( $args, array( 'reply_text' => esc_html__( 'Reply', 'teezr' ), 'depth' => $depth, 'max_depth' => $args['max_depth'] ) ), $comment->comment_ID ); ?>
					<?php edit_comment_link( esc_html__( 'Edit', 'teezr' ) ); ?>
				</small>
            </div>
            <!-- /.media-body -->
        </article>
